    ...
    <div>
      <div className="flex items-center justify-between mb-1.5">
        ...
        <button onClick={() => copy(code, k)}
          className="flex items-center gap-1.5 text-xs px-2 py-0.5 rounded-lg border border-[#334155] text-[#94A3B8] hover:text-[#F1F5F9] hover:border-[#3B82F6]/40 transition-colors">
          {copied === k ? <Check size={11} className="text-[#10B981]" /> : <Copy size={11} />}
          {copied === k ? 'Copiado' : 'Copiar'}
        </button>
      </div>
      <pre className="text-[11px] text-[#94A3B8] bg-[#0F172A] border border-[#334155] rounded-lg p-3 overflow-x-auto leading-relaxed">{code}</pre>
    </div>
  )
  ...
  return (
    <div className="bg-[#1E293B] border border-[#334155] rounded-xl p-5 space-y-4">
      <div className="flex items-center gap-2">
        ...
        <h2 className="text-sm font-semibold text-[#F1F5F9]">Webhook de eventos</h2>
      </div>
      ...
      <p className="text-[11px] text-[#64748B]">
        ...
        Alertas repetidas con el mismo <code className="text-[#94A3B8]">fingerprint</code> se correlacionan; con <code className="text-[#94A3B8]">status: resolved</code> se resuelve el incidente.
      </p>
