...
const inputCls = 'w-full px-3 py-2 rounded-lg bg-[#F4F7FB] border border-[#E6EBF2] text-[#1E293B] text-sm focus:outline-none focus:border-[#3B82F6] transition-colors placeholder-[#94A3B8]'
  ...
  return (
    <>
      <button
        ...
        className="p-1.5 rounded-lg text-[#64748B] hover:text-[#3B82F6] hover:bg-[#3B82F6]/10 transition-colors shrink-0"
      >
        <Pencil size={14} />
      </button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(15,23,42,0.45)' }} onClick={() => setOpen(false)}>
          <div className="w-full max-w-md rounded-2xl bg-white overflow-hidden" style={{ boxShadow: '0 20px 60px rgba(16,24,40,0.25)' }} onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-5 py-4" style={{ borderBottom: '1px solid #E6EBF2' }}>
              <h3 className="text-sm font-semibold text-[#1E293B]">Gestionar usuario</h3>
              <button onClick={() => setOpen(false)} className="p-1 rounded-lg text-[#94A3B8] hover:text-[#1E293B] hover:bg-[#F1F5F9]"><X size={16} /></button>
            </div>
            ...
            <div className="p-5 space-y-4 max-h-[70vh] overflow-y-auto">
              ...
              <div className="pt-4 space-y-3" style={{ borderTop: '1px solid #E6EBF2' }}>
                <h4 className="text-xs font-semibold text-[#1E293B] flex items-center gap-1.5"><KeyRound size={13} className="text-[#8B5CF6]" /> Contraseña</h4>
                ...
                {tempPass && (
                  <div className="px-3 py-2.5 rounded-lg bg-[#10B981]/10 border border-[#10B981]/20">
                    ...
                    <div className="flex items-center justify-between gap-2">
                      <code className="text-sm font-mono text-[#1E293B]">{tempPass}</code>
                      <button onClick={copyTemp} className="flex items-center gap-1 text-xs px-2 py-1 rounded-md border border-[#E6EBF2] text-[#64748B] hover:text-[#1E293B]">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={applyPass} className="px-3 py-2 rounded-lg bg-[#F1F5F9] hover:bg-[#E6EBF2] text-[#1E293B] text-sm font-medium transition-colors shrink-0">Establecer</button>
                </div>
              </div>
