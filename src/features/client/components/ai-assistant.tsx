  ...
  return (
    <>
      ...
      {!open && (
        <button
          onClick={handleOpen}
          className="fixed bottom-6 right-6 w-14 h-14 rounded-full bg-[#3B82F6] hover:bg-[#2563EB] text-white shadow-lg flex items-center justify-center transition-all hover:scale-110 z-50">
          <Sparkles size={22} />
        </button>
      )}
              ...
              <div className="flex-1 overflow-y-auto p-3 space-y-3">
                {messages.map((m, i) => (
                  <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                    {m.role === 'assistant' && (
                      <div className="w-6 h-6 rounded-full bg-[#3B82F6] flex items-center justify-center mr-2 shrink-0 mt-1">
                ...
                {flowStep === 'category_selected' && kbArticles.length > 0 && (
                  <div className="space-y-1.5 mt-1">
                    <p className="text-xs text-[#64748B] flex items-center gap-1 pl-8">
                ...
                {flowStep === 'category_selected' && !loading && messages.length >= 2 && (
                  <div className="pl-8">
                    <button
                      onClick={escalateToFreeChat}
                      className="text-xs text-[#3B82F6] hover:text-[#60A5FA] transition-colors underline underline-offset-2">
                      Necesito más ayuda →
                    </button>
                  </div>
                )}
