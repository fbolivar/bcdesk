...
export async function computeProfitability(supabase: ServerClient, f: ProfitFilters) {
  ...
  let invQ = supabase.from('invoices').select('subtotal_usd, tax_usd, total_usd, doc_type, amount_received, status, ticket_id, organization_id, issue_date').in('status', ['sent', 'overdue', 'paid'])
