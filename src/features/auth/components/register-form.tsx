...
export function RegisterForm({ token, email, orgName }: Props) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      {orgName && (
        <div className="px-3 py-2.5 rounded-lg bg-[#3B82F6]/10 border border-[#3B82F6]/20 text-sm text-[#64748B]">
          Has sido invitado a <span className="text-[#1E293B] font-medium">{orgName}</span>
        </div>
      )}
      ...
      <div>
        ...
        <input
          value={email}
          disabled
          className="w-full px-3 py-2.5 rounded-lg bg-[#FFFFFF]/50 border border-[#E6EBF2] text-[#64748B] cursor-not-allowed"
        />
      </div>

      <div>
        <label className="block text-sm font-medium text-[#64748B] mb-1.5">Nombre completo</label>
        <input
          {...register('full_name')}
          type="text"
          placeholder="Tu nombre"
          className="w-full px-3 py-2.5 rounded-lg bg-[#FFFFFF] border border-[#E6EBF2] text-[#1E293B] placeholder-[#64748B] focus:outline-none focus:border-[#3B82F6] focus:ring-1 focus:ring-[#3B82F6] transition-colors"
        />
        {errors.full_name && <p className="mt-1 text-xs text-[#EF4444]">{errors.full_name.message}</p>}
      </div>

      <div>
        <label className="block text-sm font-medium text-[#64748B] mb-1.5">Contraseña</label>
        <input
          {...register('password')}
          type="password"
      ...
      <div>
        <label className="block text-sm font-medium text-[#64748B] mb-1.5">Confirmar contraseña</label>
        <input
