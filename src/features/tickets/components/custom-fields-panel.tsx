          ...
          <div key={field.id}>
            ...
            {field.field_type === 'select' ? (
              <select value={localValues[field.id] ?? ''} onChange={e => handleChange(field.id, e.target.value)}
                className="w-full px-2.5 py-1.5 bg-[#0F172A] border border-[#334155] rounded-lg text-[#94A3B8] text-sm focus:outline-none focus:border-[#3B82F6]">
              ...
              <div className="flex gap-3">
                {['Sí', 'No'].map(opt => (
                  <label key={opt} className="flex items-center gap-1.5 cursor-pointer text-sm text-[#94A3B8]">
              ...
              <input
                ...
                className="w-full px-2.5 py-1.5 bg-[#0F172A] border border-[#334155] rounded-lg text-[#F1F5F9] text-sm focus:outline-none focus:border-[#3B82F6]"
              />
            )}
          </div>
        ))}
