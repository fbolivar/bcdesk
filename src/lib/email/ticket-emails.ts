...
function base(title: string, body: string, ctaUrl: string, ctaText: string) {
  ...
  body{font-family:system-ui,sans-serif;background:#0F172A;color:#94A3B8;margin:0;padding:20px}
  ...
  .value{font-size:14px;color:#F1F5F9;font-weight:500}
  ...
  .footer{padding:16px 24px;border-top:1px solid #334155;font-size:11px;color:#64748B}
  </style></head><body>
  <div class="card">
...
export async function sendTicketCreatedEmail(params: TicketCreatedParams) {
  ...
  const html = base(
    'Ticket recibido',
    `<p>Hola <strong style="color:#F1F5F9">${params.clientName}</strong>,</p>
...
export async function sendCommentNotificationEmail(params: CommentAddedParams) {
  ...
  const html = base(
    ...
    `<p>Hola <strong style="color:#F1F5F9">${params.recipientName}</strong>,</p>
     <p><strong style="color:#F1F5F9">${params.authorName}</strong> respondió en el ticket:</p>
     <p class="label">Ticket #${params.ticketNumber}</p><p class="value">${params.ticketTitle}</p>
     <p class="label">Mensaje</p>
     <p style="background:#0F172A;border:1px solid #334155;padding:12px;border-radius:8px;color:#F1F5F9">${params.commentPreview}</p>
...
export async function sendCsatRequestEmail(params: CsatRequestParams) {
  ...
  const html = base(
    '¿Quedaste satisfecho?',
    `<p>Hola <strong style="color:#F1F5F9">${params.clientName}</strong>,</p>
     <p>Tu ticket <strong style="color:#F1F5F9">#${params.ticketNumber} — ${params.ticketTitle}</strong> ha sido resuelto.</p>
