...
export async function GET(req: NextRequest) {
  ...
  const isClient = url.searchParams.get('type') === 'client'
  ...
  if (!isClient) resumen.push(
    ['Ingreso neto (COP)', k.netRevenue], ['Gastos (COP)', k.totalExpenses], ['Margen (COP)', k.margin], ['Margen (%)', k.marginPct ?? ''],
  )
  addSheet('Resumen', ['Indicador', 'Valor'], resumen)
