...
export default async function SurveyPage({ params }: { params: { id: string } }) {
  ...
  if (existing) {
    return (
      <div className="max-w-lg mx-auto mt-20 text-center space-y-4">
        ...
        <h2 className="text-xl font-semibold text-[#F1F5F9]">¡Gracias por tu respuesta!</h2>
        <p className="text-[#94A3B8]">Ya respondiste esta encuesta.</p>
      </div>
    )
  }
  ...
  async function handleSubmit(formData: FormData) {
    ...
    revalidatePath(`/client/survey/${params.id}`)
    redirect(`/client/survey/${params.id}`)
  }
  ...
  return (
    <div className="max-w-lg mx-auto mt-12 space-y-6">
      <div className="text-center">
        ...
        <h1 className="text-xl font-semibold text-[#F1F5F9]">{survey.title}</h1>
        {survey.description && <p className="text-sm text-[#94A3B8] mt-1">{survey.description}</p>}
      </div>

      <form action={handleSubmit} className="bg-[#1E293B] border border-[#334155] rounded-xl p-6 space-y-6">
        <div>
            ...
            {Array.from({ length: scale }, (_, i) => (
              <label key={i} className="cursor-pointer">
                ...
                <span className={`flex items-center justify-center w-10 h-10 rounded-lg border text-sm font-medium transition-all
                  ...
                  ${isNps && i >= 9 ? 'border-[#10B981]/50 text-[#10B981]' : isNps && i >= 7 ? 'border-[#F59E0B]/50 text-[#F59E0B]' : 'border-[#334155] text-[#94A3B8]'}
        ...
        <div>
          <label className="block text-xs text-[#94A3B8] mb-1">Comentario adicional (opcional)</label>
