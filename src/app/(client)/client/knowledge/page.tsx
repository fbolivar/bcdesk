  ...
  return (
    <div className="max-w-3xl space-y-6">
      <div>
        ...
        <p className="text-sm text-[#5B6B7C] mt-0.5">Encuentra respuestas a tus preguntas</p>
      </div>

      <form method="GET">
        <div className="relative">
          <Search size={16} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[#5B6B7C]" />
          <input
            name="q"
            defaultValue={q}
            placeholder="Buscar en la base de conocimiento..."
            className="w-full pl-10 pr-4 py-3 rounded-xl text-[#0B2545] text-sm focus:outline-none placeholder-[#5B6B7C]"
            style={{
              background: '#FFFFFF',
      ...
      {list.length === 0 ? (
        ...
        >
          <BookOpen size={40} className="text-[#5B6B7C] mb-3" />
          <p className="text-[#0B2545] font-medium">
            {q ? 'Sin resultados para esta búsqueda' : 'Sin artículos disponibles'}
          </p>
          {q && <p className="text-sm text-[#5B6B7C] mt-1">Intenta con otras palabras clave</p>}
        </div>
      ) : q ? (
        ...
        Object.entries(byCategory).map(([catName, arts]) => (
          <div key={catName}>
            <h2 className="text-xs font-semibold text-[#5B6B7C] uppercase tracking-wider mb-3">{catName}</h2>
