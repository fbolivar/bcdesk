  ...
  return (
    <div className="max-w-4xl space-y-6">
      <div>
        ...
        <p className="text-sm text-[#94A3B8] mt-0.5">Crea y aprueba compras con flujos de aprobación multinivel.</p>
      </div>
      ...
      <div className="bg-[#1E293B] border border-[#334155] rounded-xl overflow-hidden">
        ...
        {(purchases ?? []).map(p => (
          <Link key={p.id} href={`/admin/purchases/${p.id}`}
            className="flex items-center gap-3 px-4 py-3 border-b border-[#334155]/50 last:border-0 hover:bg-[#0F172A]/50 transition-colors">
            ...
            <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium shrink-0 ${STATUS_COLOR[p.status] ?? 'bg-[#334155] text-[#94A3B8]'}`}>
        ...
        <form action={createPurchaseRequest} className="space-y-4">
          <div>
            ...
            <input name="title" required placeholder="Ej: 10 licencias Adobe Creative Cloud"
              className="w-full px-3 py-2 rounded-lg bg-[#0F172A] border border-[#334155] text-[#F1F5F9] text-sm focus:outline-none focus:border-[#3B82F6] placeholder-[#64748B]" />
          </div>
          <div>
            <label className="block text-xs font-medium text-[#94A3B8] mb-1.5">Descripción / justificación</label>
            <textarea name="description" rows={3} placeholder="Motivo de la compra…"
              className="w-full px-3 py-2 rounded-lg bg-[#0F172A] border border-[#334155] text-[#F1F5F9] text-sm focus:outline-none focus:border-[#3B82F6] placeholder-[#64748B] resize-none" />
          </div>
          <div className="grid grid-cols-3 gap-3">
            <div className="col-span-1">
              <label className="block text-xs font-medium text-[#94A3B8] mb-1.5">Proveedor</label>
              <input name="vendor" placeholder="Proveedor"
                className="w-full px-3 py-2 rounded-lg bg-[#0F172A] border border-[#334155] text-[#F1F5F9] text-sm focus:outline-none focus:border-[#3B82F6] placeholder-[#64748B]" />
            </div>
            <div>
              <label className="block text-xs font-medium text-[#94A3B8] mb-1.5">Monto</label>
