...
export default async function MajorIncidentDetailPage({ params }: { params: { id: string } }) {
  ...
  const [updatesRes, linkedTicketsRes, availableTicketsRes] = await Promise.all([
    supabase.from('major_incident_updates')
      ...
      .eq('major_incident_id', params.id)
  ...
  async function handleAddUpdate(formData: FormData) {
    ...
    revalidatePath(`/admin/major-incidents/${params.id}`)
  }
    ...
    <div className="space-y-6 max-w-5xl">
      <div className="flex items-start justify-between">
        <div>
          ...
          <h1 className="text-xl font-semibold text-[#F1F5F9]">{incident.title}</h1>
          {incident.description && <p className="text-sm text-[#94A3B8] mt-1">{incident.description}</p>}
        </div>
        <div className="text-right text-xs text-[#64748B]">
          <p>Comandante: <span className="text-[#94A3B8]">{commander?.full_name || commander?.email || '—'}</span></p>
            ...
            <div key={s} className="flex items-center flex-1">
              <div className={`flex flex-col items-center ${idx < STATUS_STEPS.length - 1 ? 'flex-1' : ''}`}>
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold border-2
                  ${idx <= currentStep ? 'bg-[#3B82F6] border-[#3B82F6] text-white' : 'bg-[#0F172A] border-[#334155] text-[#475569]'}`}>
                  {idx < currentStep ? <CheckCircle2 size={14} /> : idx + 1}
                </div>
                <p className="text-xs text-[#64748B] mt-1">{STATUS_LABEL[s]}</p>
              </div>
              {idx < STATUS_STEPS.length - 1 && (
                <div className={`flex-1 h-0.5 mx-2 ${idx < currentStep ? 'bg-[#3B82F6]' : 'bg-[#334155]'}`} />
              )}
            </div>
          ))}
        </div>
      ...
      <div className="grid grid-cols-2 gap-6">
        {/* Post update */}
        <div className="bg-[#1E293B] border border-[#334155] rounded-xl p-5">
          <h2 className="text-sm font-semibold text-[#F1F5F9] mb-4">Publicar actualización</h2>
          <form action={handleAddUpdate} className="space-y-3">
            ...
            <div>
              <label className="block text-xs text-[#94A3B8] mb-1">Cambiar estado</label>
        ...
        {/* Link tickets */}
        <div className="bg-[#1E293B] border border-[#334155] rounded-xl p-5">
          <h2 className="text-sm font-semibold text-[#F1F5F9] mb-4">Vincular tickets afectados</h2>
          {availableTickets.length > 0 ? (
            <div className="space-y-1 max-h-60 overflow-y-auto">
              {availableTickets.map(t => (
                <form key={t.id} action={handleLinkTicket.bind(null, t.id)}
                  className="flex items-center gap-2 px-2 py-1.5 rounded hover:bg-[#263248] group">
                  <div className="flex-1">
                    <p className="text-xs font-medium text-[#F1F5F9] truncate">{t.title}</p>
                    <p className="text-[10px] text-[#475569]">{t.status} · {t.priority}</p>
                  </div>
                  <button type="submit"
          ...
          ) : <p className="text-xs text-[#475569]">No hay tickets disponibles para vincular.</p>}
            ...
            <div className="mt-4">
              ...
              <div className="space-y-1">
                {(linkedTicketsRes.data as any[]).map(lt => (
                  <div key={lt.id} className="flex items-center gap-2 px-2 py-1.5 rounded bg-[#0F172A]">
                    <span className="text-xs text-[#94A3B8] truncate">{lt.tickets?.title}</span>
                  </div>
                ))}
              </div>
            </div>
      ...
      {updates.length > 0 && (
        <div className="bg-[#1E293B] border border-[#334155] rounded-xl p-5">
          <h2 className="text-sm font-semibold text-[#F1F5F9] mb-4">Timeline del incidente</h2>
              ...
              return (
                <div key={u.id} className="flex gap-3">
                  <div className="flex flex-col items-center">
                    ...
                    <div className="flex-1 w-px bg-[#334155] mt-1" />
                  </div>
                  <div className="flex-1 pb-4">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-xs font-medium text-[#94A3B8]">{author?.full_name || 'Sistema'}</span>
                      <span className="text-[10px] text-[#475569]">{new Date(u.created_at).toLocaleString('es-CO')}</span>
                    ...
                    <p className="text-sm text-[#F1F5F9]">{u.update_text}</p>
                  </div>
                </div>
              )
            })}
