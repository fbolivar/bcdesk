...
export default async function ReleasesPage() {
  ...
  return (
    <div className="space-y-6 max-w-5xl">
      <div>
        <h1 className="text-xl font-semibold text-[#1E293B]">Gestión de releases</h1>
        <p className="text-sm text-[#64748B] mt-0.5">ITIL Release Management — paquetes de cambios para despliegue</p>
      </div>

      {/* Create */}
      <div className="bg-[#FFFFFF] border border-[#E6EBF2] rounded-xl p-5">
        <h2 className="text-sm font-semibold text-[#1E293B] mb-4">Nuevo release</h2>
        <form action={handleCreate} className="grid grid-cols-3 gap-3">
          ...
          <div>
            <label className="block text-xs text-[#64748B] mb-1">Versión *</label>
          ...
          <div>
            <label className="block text-xs text-[#64748B] mb-1">Ambiente</label>
          ...
          <div>
            <label className="block text-xs text-[#64748B] mb-1">Fecha planificada</label>
            ...
            <tbody>
              {list.map((r: any) => (
                <tr key={r.id} className="border-b border-[#E6EBF2]/50 hover:bg-[#EEF2F7]">
                  ...
                  <td className="px-4 py-3 text-xs font-mono text-[#64748B]">v{r.version}</td>
                  <td className="px-4 py-3">
                    <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${TYPE_COLOR[r.release_type]}`}>
                      {r.release_type}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-xs text-[#64748B]">{r.environment}</td>
                  <td className="px-4 py-3">
                    <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${STATUS_COLOR[r.status]}`}>
                      {r.status}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-xs text-[#64748B]">{r.release_changes?.[0]?.count ?? 0}</td>
