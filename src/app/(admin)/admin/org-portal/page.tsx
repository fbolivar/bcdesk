...
export default async function OrgPortalPage() {
  ...
  return (
    <div className="space-y-6 max-w-5xl">
      <div>
        <h1 className="text-xl font-semibold text-[#F1F5F9]">Portal de organizaciones</h1>
        <p className="text-sm text-[#94A3B8] mt-0.5">Vista consolidada de todas las organizaciones cliente</p>
      </div>

      <div className="grid grid-cols-1 gap-4">
        {orgList.map((org: any) => (
          <div key={org.id} className="bg-[#1E293B] border border-[#334155] rounded-xl p-5">
            <div className="flex items-start justify-between mb-4">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl bg-[#334155] flex items-center justify-center text-[#F1F5F9] font-bold">
                  {org.name.charAt(0)}
                </div>
                <div>
                  <h2 className="font-semibold text-[#F1F5F9]">{org.name}</h2>
                  {org.domain && <p className="text-xs text-[#64748B]">{org.domain}</p>}
                </div>
              </div>
              <Link href={`/admin/org-portal/${org.id}`}
                className="px-3 py-1.5 rounded-lg bg-[#334155] hover:bg-[#475569] text-[#F1F5F9] text-xs transition-colors">
                Ver detalle
              </Link>
            </div>
            <div className="grid grid-cols-3 gap-3">
              <div className="bg-[#0F172A] rounded-lg p-3 flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="font-semibold text-[#F1F5F9]">{org.tickets?.[0]?.count ?? 0}</p>
                </div>
              </div>
              <div className="bg-[#0F172A] rounded-lg p-3 flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="font-semibold text-[#F1F5F9]">{org.profiles?.[0]?.count ?? 0}</p>
                </div>
              </div>
              <div className="bg-[#0F172A] rounded-lg p-3 flex items-center gap-2">
