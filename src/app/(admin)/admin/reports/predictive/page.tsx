...
export default async function PredictivePage() {
  ...
  return (
    <div className="space-y-6 max-w-5xl">
      <div>
        <h1 className="text-xl font-semibold text-[#F1F5F9]">Analítica predictiva</h1>
        <p className="text-sm text-[#94A3B8] mt-0.5">Forecast de volumen de tickets y detección de tendencias — últimas 12 semanas</p>
      </div>
      ...
      <div className="grid grid-cols-4 gap-4">
        <div className="bg-[#1E293B] border border-[#334155] rounded-xl p-4">
          ...
          <p className="text-2xl font-bold text-[#F1F5F9]">{avgWeekly}</p>
          <p className="text-xs text-[#475569] mt-0.5">tickets/semana</p>
        </div>
        <div className="bg-[#1E293B] border border-[#334155] rounded-xl p-4">
          <p className="text-xs text-[#64748B] mb-1">Tendencia</p>
          <p className={`text-2xl font-bold ${trendColor}`}>{trend}</p>
          <p className="text-xs text-[#475569] mt-0.5">{slope > 0 ? `+${slope.toFixed(1)}` : slope.toFixed(1)} tickets/sem</p>
        </div>
        <div className="bg-[#1E293B] border border-[#334155] rounded-xl p-4">
          ...
          <p className="text-xs text-[#475569] mt-0.5">{totalResolved}/{totalCreated} tickets</p>
        </div>
        <div className="bg-[#1E293B] border border-[#334155] rounded-xl p-4">
          <p className="text-xs text-[#64748B] mb-1">Día más ocupado</p>
          <p className="text-2xl font-bold text-[#F1F5F9]">{busiestDow ? dowNames[parseInt(busiestDow[0])] : '—'}</p>
          <p className="text-xs text-[#475569] mt-0.5">{busiestDow ? `${busiestDow[1]} tickets histórico` : ''}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-6">
        ...
        <div className="bg-[#1E293B] border border-[#334155] rounded-xl p-5">
          <h2 className="text-sm font-semibold text-[#F1F5F9] mb-4">Volumen histórico (12 semanas)</h2>
          {weeks.length > 0 ? (
              ...
              {weeks.slice(-8).map((w, i) => {
                ...
                return (
                  <div key={w} className="flex items-center gap-3">
                    <span className="text-xs text-[#475569] w-20 shrink-0">{w.slice(5)}</span>
            ...
            <p className="text-xs text-[#475569]">Sin datos suficientes.</p>
          )}
        </div>

        {/* Forecast */}
        <div className="bg-[#1E293B] border border-[#334155] rounded-xl p-5">
          <h2 className="text-sm font-semibold text-[#F1F5F9] mb-1">Forecast próximas 4 semanas</h2>
          <p className="text-xs text-[#475569] mb-4">Regresión lineal sobre histórico de 12 semanas</p>
              ...
              return (
                <div key={f.week}>
                  <div className="flex items-center justify-between text-xs mb-1">
                    <span className="text-[#94A3B8]">Sem. {f.week.slice(5)}</span>
                    <span className={`font-semibold ${isHigh ? 'text-[#F59E0B]' : 'text-[#F1F5F9]'}`}>
        ...
        <div className="bg-[#1E293B] border border-[#334155] rounded-xl p-5">
          ...
          <div className="grid grid-cols-5 gap-3">
            {topCategories.map(([cat, count]) => (
              <div key={cat} className="bg-[#0F172A] rounded-xl p-3 text-center">
                <p className="text-lg font-bold text-[#F1F5F9]">{count}</p>
