...
export default async function ChangeDetailPage({ params }: Props) {
  ...
  return (
    <div className="max-w-5xl space-y-6">
      <Link href="/admin/changes" className="flex items-center gap-2 text-sm text-[#5B6B7C] hover:text-[#0B2545] transition-colors w-fit">
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1 min-w-0">
          <h1 className="text-xl font-semibold text-[#0B2545] truncate">{change.title}</h1>
          <div className="flex items-center gap-2 mt-2 flex-wrap">
            <span className={`px-2.5 py-0.5 rounded-full text-xs font-medium ${TYPE_COLOR[change.change_type] ?? 'bg-[#E6EBF2] text-[#5B6B7C]'}`}>
              {TYPE_LABEL[change.change_type] ?? change.change_type}
            </span>
            <span className={`px-2.5 py-0.5 rounded-full text-xs font-medium ${STATUS_COLOR[change.status] ?? 'bg-[#E6EBF2] text-[#5B6B7C]'}`}>
              {STATUS_LABEL[change.status] ?? change.status}
            </span>
            <span className={`px-2.5 py-0.5 rounded-full text-xs font-medium ${RISK_COLOR[change.risk_level] ?? 'bg-[#E6EBF2] text-[#5B6B7C]'}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-3 space-y-4">
          ...
          <div className="bg-[#FFFFFF] border border-[#E6EBF2] rounded-xl p-5 space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-3 text-sm">
              {creator && (
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[#5B6B7C]">Creado por:</span>
                  <span className="text-[#0B2545]">{creator.full_name ?? '—'}</span>
                </div>
              )}
              {change.planned_start && (
                <div className="flex items-center gap-2">
                  <Calendar size={13} className="text-[#5B6B7C]" />
                  <span className="text-[#5B6B7C]">Inicio:</span>
                  <span className="text-[#0B2545] text-xs">
              ...
              {change.planned_end && (
                <div className="flex items-center gap-2">
                  <Calendar size={13} className="text-[#5B6B7C]" />
                  <span className="text-[#5B6B7C]">Fin:</span>
                  <span className="text-[#0B2545] text-xs">
            ...
            {change.description && (
              <div className="space-y-1">
                <p className="text-xs font-medium text-[#5B6B7C] uppercase tracking-wide">Descripción / Justificación</p>
                <p className="text-sm text-[#0B2545] whitespace-pre-wrap leading-relaxed">{change.description}</p>
              </div>
            )}

            {change.rollback_plan && (
              <div className="space-y-1">
                <p className="text-xs font-medium text-[#5B6B7C] uppercase tracking-wide">Plan de rollback</p>
                <p className="text-sm text-[#0B2545] whitespace-pre-wrap leading-relaxed">{change.rollback_plan}</p>
              </div>
            )}
          </div>
          ...
          {(approvals ?? []).length > 0 && (
            <div className="bg-[#FFFFFF] border border-[#E6EBF2] rounded-xl p-5 space-y-3">
              <h2 className="text-sm font-semibold text-[#0B2545]">Historial de decisiones CAB</h2>
              {(approvals ?? []).map((a: Record<string, unknown>) => {
                ...
                return (
                  <div key={a.id as string} className="flex items-start justify-between gap-3 px-3 py-2.5 bg-[#F4F7FB] rounded-lg">
                    <div className="flex-1 min-w-0">
                      <p className="text-sm text-[#0B2545]">{approver?.full_name as string ?? '—'}</p>
                      {comment && <p className="text-xs text-[#5B6B7C] mt-0.5">{comment}</p>}
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="bg-[#FFFFFF] border border-[#E6EBF2] rounded-xl p-5 space-y-5">
            <h2 className="text-sm font-semibold text-[#0B2545]">Flujo CAB</h2>
                  ...
                  return (
                    <div key={step.key} className="flex items-start gap-3">
                      ...
                      <p className={`text-sm pt-0.5 ${
                        isActive ? 'text-[#0B2545] font-medium' :
                        isDone ? 'text-[#10B981]' :
                        'text-[#CBD5E1]'
                      }`}>
                        {step.label}
            ...
            {isRejectedOrCancelled && (
              <div className={`flex items-center gap-2 px-3 py-2 rounded-lg ${
                ...
                <XCircle size={14} className={change.status === 'rejected' ? 'text-[#EF4444]' : 'text-[#5B6B7C]'} />
                <span className="text-sm text-[#0B2545]">
            ...
            <div className="border-t border-[#E6EBF2] pt-4 space-y-3">
              <p className="text-xs font-medium text-[#5B6B7C] uppercase tracking-wide">Acciones</p>
              ...
              {change.status === 'submitted' && approvalState && approvalState.status === 'pending' && (
                approvalState.canActNow ? (
                  <div className="space-y-2">
                    <p className="text-[11px] text-[#5B6B7C]">
                      Paso {approvalState.currentStep}: <span className="text-[#0B2545]">{approvalState.steps[approvalState.currentStep - 1]?.step.name}</span> — tu decisión:
                    </p>
                  ...
                  <p className="text-xs text-[#5B6B7C] text-center py-2">
              ...
              {change.status === 'rejected' && (
                <form action={reopenAction}>
                  <button type="submit"
                    className="w-full flex items-center justify-center gap-2 px-4 py-2 rounded-lg bg-[#E6EBF2] hover:bg-[#CBD5E1] text-[#5B6B7C] text-sm font-medium transition-colors">
                    Volver a borrador
                  </button>
                </form>
              )}
              ...
              {['draft', 'submitted', 'approved', 'in_progress'].includes(change.status) && (
                <form action={cancelAction}>
                  <button type="submit"
                    className="w-full flex items-center justify-center gap-2 px-4 py-2 rounded-lg bg-transparent hover:bg-[#EF4444]/10 text-[#5B6B7C] hover:text-[#EF4444] text-xs transition-colors border border-[#E6EBF2] hover:border-[#EF4444]/30">
                    Cancelar cambio
                  </button>
                </form>
              )}
