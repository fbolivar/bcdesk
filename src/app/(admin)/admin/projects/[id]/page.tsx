...
export default async function AdminProjectDetailPage({ params }: Props) {
  ...
  return (
    <div className="max-w-3xl space-y-6">
      <Link href="/admin/projects" className="inline-flex items-center gap-2 text-sm text-[#64748B] hover:text-[#1E293B]">
        <ArrowLeft size={14} /> Volver a proyectos
      </Link>

      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-xl font-semibold text-[#1E293B]">{p.name}</h1>
          <p className="text-sm text-[#64748B] mt-0.5">{p.organizations?.name}</p>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          <div key={s.label} className="bg-[#FFFFFF] border border-[#E6EBF2] rounded-xl p-3">
            ...
            <p className="text-sm font-medium text-[#1E293B]">{s.value}</p>
          </div>
        ))}
      </div>
      ...
      <div className="bg-[#FFFFFF] border border-[#E6EBF2] rounded-xl p-5">
        <div className="flex items-center justify-between mb-2">
          <h2 className="text-sm font-semibold text-[#1E293B]">Progreso general</h2>
      ...
      {/* Phases */}
      <div>
        <h2 className="text-sm font-semibold text-[#1E293B] mb-3">Fases ({phases.length})</h2>
        {phases.length === 0 ? (
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2">
                        <p className="text-sm font-medium text-[#1E293B]">{phase.name}</p>
        ...
        <details className="bg-[#FFFFFF] border border-[#E6EBF2] rounded-xl mt-3">
          <summary className="px-4 py-3 cursor-pointer text-sm font-medium text-[#64748B] hover:text-[#1E293B] select-none">
            + Agregar fase
          </summary>
          <form action={handleAddPhase} className="px-4 pb-4 space-y-3 border-t border-[#E6EBF2] pt-3">
            <div className="grid grid-cols-2 gap-3">
              <div className="col-span-2">
                <label className="block text-xs font-medium text-[#64748B] mb-1">Nombre *</label>
              ...
              <div>
                <label className="block text-xs font-medium text-[#64748B] mb-1">Fecha inicio</label>
              ...
              <div>
                <label className="block text-xs font-medium text-[#64748B] mb-1">Fecha fin</label>
      ...
      {/* Kanban board */}
      <div>
        <h2 className="text-sm font-semibold text-[#1E293B] mb-3">Tareas ({tasks.length})</h2>
