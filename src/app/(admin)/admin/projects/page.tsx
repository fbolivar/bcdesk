  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm text-[#94A3B8] mt-0.5">{typedProjects.length} proyectos</p>
      </div>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {typedProjects.length === 0 ? (
              ...
              return (
                <tr key={p.id} className="border-b border-[#334155]/50 hover:bg-[#263248] transition-colors">
                  <td className="px-4 py-3">
                    <Link href={`/admin/projects/${p.id}`} className="text-sm font-medium text-[#F1F5F9] hover:text-[#3B82F6]">{p.name}</Link>
                    {p.description && <p className="text-xs text-[#64748B] mt-0.5 line-clamp-1">{p.description}</p>}
                  </td>
                  <td className="px-4 py-3 text-xs text-[#94A3B8]">{p.organizations?.name ?? '—'}</td>
                  ...
                  <td className="px-4 py-3 min-w-[100px]">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-[#94A3B8] w-8 text-right">{p.progress_percent}%</span>
                    </div>
                  </td>
      ...
      <details className="bg-[#1E293B] border border-[#334155] rounded-xl">
        ...
        <form action={createProject} className="px-5 pb-5 space-y-4 border-t border-[#334155] pt-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs font-medium text-[#94A3B8] mb-1.5">Presupuesto (USD)</label>
              <input name="budget_usd" type="number" step="0.01" className="w-full px-3 py-2 rounded-lg bg-[#0F172A] border border-[#334155] text-[#F1F5F9] text-sm focus:outline-none focus:border-[#3B82F6] transition-colors" />
            </div>
            <div>
              <label className="block text-xs font-medium text-[#94A3B8] mb-1.5">Fecha inicio</label>
            ...
            <div>
              <label className="block text-xs font-medium text-[#94A3B8] mb-1.5">Fecha fin estimada</label>
          ...
          <div>
            ...
            <textarea name="description" rows={2} className="w-full px-3 py-2 rounded-lg bg-[#0F172A] border border-[#334155] text-[#F1F5F9] text-sm focus:outline-none focus:border-[#3B82F6] transition-colors resize-none" />
          </div>
