...
export default async function ProblemDetailPage({ params }: Props) {
  ...
  return (
    <div className="max-w-3xl space-y-6">
      <Link href="/admin/problems" className="flex items-center gap-2 text-sm text-[#64748B] hover:text-[#1E293B]">
        <ArrowLeft size={14} /> Volver a problemas
      </Link>

      <h1 className="text-xl font-semibold text-[#1E293B]">{problem.title}</h1>

      {/* Edit form */}
      <div className="bg-[#FFFFFF] border border-[#E6EBF2] rounded-xl p-5 space-y-4">
        <h2 className="text-sm font-semibold text-[#1E293B]">Detalles del problema</h2>
        <form action={updateProblem.bind(null, id)} className="space-y-3">
          ...
          <div>
            <label className="block text-xs text-[#64748B] mb-1">Descripción / Síntomas</label>
            <textarea name="description" rows={3} defaultValue={problem.description ?? ''}
              className="w-full px-3 py-2 bg-[#F4F7FB] border border-[#E6EBF2] rounded-lg text-[#1E293B] text-sm focus:outline-none focus:border-[#3B82F6] resize-none" />
          </div>
          <div>
            <label className="block text-xs text-[#64748B] mb-1">Causa raíz</label>
          ...
          <div>
            <label className="block text-xs text-[#64748B] mb-1">Solución temporal (workaround)</label>
      ...
      <div className="bg-[#FFFFFF] border border-[#E6EBF2] rounded-xl p-5 space-y-3">
        <h2 className="text-sm font-semibold text-[#1E293B]">Incidentes vinculados ({linkedTickets?.length ?? 0})</h2>
            ...
            {linkedTickets!.map((pi: any) => {
              ...
              return (
                <div key={pi.ticket_id} className="flex items-center justify-between px-3 py-2 bg-[#F4F7FB] rounded-lg">
                  <Link href={`/admin/tickets/${pi.ticket_id}`} className="text-sm text-[#1E293B] hover:text-[#3B82F6]">
        ...
        {(openTickets ?? []).length > 0 && (
          <form action={async (fd: FormData) => {
            ...
            <select name="ticket_id"
              className="flex-1 px-3 py-2 bg-[#F4F7FB] border border-[#E6EBF2] rounded-lg text-[#1E293B] text-sm focus:outline-none focus:border-[#3B82F6]">
            ...
            <button type="submit"
              className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-[#E6EBF2] hover:bg-[#CBD5E1] text-[#1E293B] text-sm transition-colors">
