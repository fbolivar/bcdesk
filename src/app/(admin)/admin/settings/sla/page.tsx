...
export default async function AdminSlaPage() {
  ...
  return (
    <div className="max-w-4xl space-y-6">
      <div>
        <h1 className="text-xl font-semibold text-[#F1F5F9]">Políticas SLA</h1>
      ...
      {Object.entries(grouped).map(([category, items]) => (
        <div key={category} className="bg-[#1E293B] border border-[#334155] rounded-xl overflow-hidden">
          <div className="px-5 py-3 border-b border-[#334155] flex items-center gap-2">
            ...
            <h2 className="text-sm font-semibold text-[#F1F5F9]">{CATEGORY_LABEL[category] ?? category}</h2>
            ...
            {(items ?? []).map(policy => (
              <details key={policy.id} className="group">
                <summary className="flex items-center gap-3 px-5 py-3 cursor-pointer hover:bg-[#263248] transition-colors list-none">
                  ...
                  <span className="text-sm font-medium text-[#F1F5F9] flex-1">{policy.name}</span>
                ...
                <div className="px-5 pb-5 pt-3 border-t border-[#334155]/50 bg-[#0F172A]/30">
