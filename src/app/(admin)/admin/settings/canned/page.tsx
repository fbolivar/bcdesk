  ...
  return (
    <div className="max-w-4xl space-y-6">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-xl font-semibold text-[#1E293B]">Respuestas rápidas</h1>
      ...
      <details className="bg-[#FFFFFF] border border-[#E6EBF2] rounded-xl">
        ...
        <form action={createCannedResponse} className="px-5 pb-5 pt-3 border-t border-[#E6EBF2] space-y-3">
          ...
          <div>
            <label className="block text-xs font-medium text-[#64748B] mb-1.5">Contenido *</label>
            <textarea name="content" required rows={3} placeholder="Texto de la respuesta..."
              className="w-full px-3 py-2 rounded-lg bg-[#F4F7FB] border border-[#E6EBF2] text-[#1E293B] text-sm focus:outline-none focus:border-[#3B82F6] transition-colors resize-none placeholder-[#64748B]" />
          </div>
          <button type="submit"
      ...
      {Object.entries(grouped).map(([category, items]) => (
        <div key={category} className="bg-[#FFFFFF] border border-[#E6EBF2] rounded-xl overflow-hidden">
          <div className="px-5 py-3 border-b border-[#E6EBF2] flex items-center gap-2">
            ...
            <h2 className="text-sm font-semibold text-[#1E293B] capitalize">{CATEGORY_LABELS[category] ?? category}</h2>
            ...
            {(items ?? []).map(r => (
              <details key={r.id} className="group">
                <summary className="flex items-center gap-3 px-5 py-3 cursor-pointer hover:bg-[#EEF2F7] transition-colors list-none">
                  ...
                  <span className="text-sm font-medium text-[#1E293B] flex-1">{r.title}</span>
                ...
                <div className="px-5 pb-4 pt-3 border-t border-[#E6EBF2]/50 bg-[#F4F7FB]/30 space-y-3">
                  <form action={updateCannedResponse} className="space-y-3">
                    ...
                    <div>
                      <label className="block text-xs font-medium text-[#64748B] mb-1">Contenido</label>
                      <textarea name="content" defaultValue={r.content} rows={3}
                        className="w-full px-3 py-2 rounded-lg bg-[#F4F7FB] border border-[#E6EBF2] text-[#1E293B] text-sm focus:outline-none focus:border-[#3B82F6] transition-colors resize-none" />
                    </div>
                    <button type="submit"
                      className="px-4 py-2 rounded-lg bg-[#3B82F6] hover:bg-[#2563EB] text-white text-sm font-medium transition-colors">
                      Guardar cambios
