    ...
    <div className="max-w-4xl space-y-6">
      ...
      <div className="bg-[#1E293B] border border-[#334155] rounded-xl overflow-hidden">
        <div className="px-4 py-3 border-b border-[#334155] flex items-center gap-2">
          ...
          <h2 className="text-sm font-semibold text-[#F1F5F9]">Usuarios sincronizados desde el directorio ({syncedList.length})</h2>
        </div>
        {syncedList.length === 0 && (
        ...
        {syncedList.map(p => (
          <div key={p.id} className="flex items-center gap-3 px-4 py-3 border-b border-[#334155]/50 last:border-0">
            <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-medium shrink-0 ${p.is_active ? 'bg-[#334155] text-[#F1F5F9]' : 'bg-[#1E293B] border border-[#334155] text-[#64748B]'}`}>
              {p.full_name?.charAt(0)?.toUpperCase() ?? '?'}
            </div>
            <div className="flex-1 min-w-0">
              <p className={`text-sm ${p.is_active ? 'text-[#F1F5F9]' : 'text-[#64748B] line-through'}`}>{p.full_name}</p>
              <p className="text-xs text-[#64748B]">{p.email}</p>
            </div>
            <span className="text-[10px] px-2 py-0.5 rounded-full bg-[#334155] text-[#94A3B8]">{ROLE_LABEL[p.role] ?? p.role}</span>
