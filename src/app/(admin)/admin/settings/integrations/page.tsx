...
export default async function IntegrationsPage() {
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div>
        <h1 className="text-xl font-semibold text-[#F1F5F9]">Integraciones</h1>
        <p className="text-sm text-[#94A3B8] mt-0.5">Conecta BCDesk con Slack, Teams, WhatsApp y webhooks</p>
      </div>
      ...
      <div className="flex items-start gap-3 px-4 py-3 bg-[#3B82F6]/10 border border-[#3B82F6]/20 rounded-xl">
        ...
        <p className="text-xs text-[#94A3B8]">
          Para <strong className="text-[#F1F5F9]">Slack</strong>: crea una Incoming Webhook en api.slack.com/apps · Para <strong className="text-[#F1F5F9]">Teams</strong>: usa un Incoming Webhook connector en tu canal · Para <strong className="text-[#F1F5F9]">WhatsApp</strong>: conecta via Twilio o Meta Business API
        </p>
      </div>

      {/* Create */}
      <div className="bg-[#1E293B] border border-[#334155] rounded-xl p-5">
        <h2 className="text-sm font-semibold text-[#F1F5F9] mb-4">Nueva integración</h2>
        <form action={handleCreate} className="space-y-3">
          ...
          <div>
            <label className="block text-xs text-[#94A3B8] mb-1">URL del Webhook *</label>
            <input name="webhook_url" required type="url" placeholder="https://hooks.slack.com/services/..."
              className="w-full px-3 py-2 bg-[#0F172A] border border-[#334155] rounded-lg text-[#F1F5F9] text-sm focus:outline-none focus:border-[#3B82F6] placeholder-[#475569]" />
          </div>
          <div>
            <label className="block text-xs text-[#94A3B8] mb-2">Eventos que disparan la notificación</label>
      ...
      {list.length > 0 && (
        <div className="bg-[#1E293B] border border-[#334155] rounded-xl overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody>
              {list.map((i: any) => (
                <tr key={i.id} className="border-b border-[#334155]/50 hover:bg-[#263248]">
                  <td className="px-4 py-3">
                    <p className="font-medium text-[#F1F5F9]">{i.name}</p>
                    <p className="text-[10px] text-[#475569] font-mono truncate max-w-[160px]">{i.webhook_url}</p>
                  </td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3">
                    <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${i.is_active ? 'bg-[#10B981]/20 text-[#10B981]' : 'bg-[#334155] text-[#64748B]'}`}>
