...
export default async function RolesPage() {
  ...
  return (
    <div className="space-y-6 max-w-5xl">
      <div>
        <h1 className="text-xl font-semibold text-[#1E293B]">Roles personalizados</h1>
        <p className="text-sm text-[#64748B] mt-0.5">Define permisos granulares más allá de admin/agente/cliente</p>
      </div>

      {/* Create */}
      <div className="bg-[#FFFFFF] border border-[#E6EBF2] rounded-xl p-5">
        <h2 className="text-sm font-semibold text-[#1E293B] mb-4">Nuevo rol</h2>
        <form action={handleCreate} className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs text-[#64748B] mb-1">Nombre del rol *</label>
          ...
          <div>
            <label className="block text-xs text-[#64748B] mb-2">Permisos</label>
            <div className="grid grid-cols-3 gap-2">
              {ALL_PERMISSIONS.map(p => (
                <label key={p.key} className="flex items-center gap-2 text-xs text-[#64748B] cursor-pointer px-2 py-1.5 rounded hover:bg-[#EEF2F7]">
            ...
            return (
              <div key={role.id} className="bg-[#FFFFFF] border border-[#E6EBF2] rounded-xl p-4">
                <div className="flex items-start justify-between">
                  <div>
                    <h3 className="font-semibold text-[#1E293B]">{role.name}</h3>
