...
export default async function RolesPage() {
  ...
  return (
    <div className="space-y-6 max-w-5xl">
      <div>
        <h1 className="text-xl font-semibold text-[#F1F5F9]">Roles personalizados</h1>
        <p className="text-sm text-[#94A3B8] mt-0.5">Define permisos granulares más allá de admin/agente/cliente</p>
      </div>

      {/* Create */}
      <div className="bg-[#1E293B] border border-[#334155] rounded-xl p-5">
        <h2 className="text-sm font-semibold text-[#F1F5F9] mb-4">Nuevo rol</h2>
        <form action={handleCreate} className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs text-[#94A3B8] mb-1">Nombre del rol *</label>
          ...
          <div>
            <label className="block text-xs text-[#94A3B8] mb-2">Permisos</label>
            <div className="grid grid-cols-3 gap-2">
              {ALL_PERMISSIONS.map(p => (
                <label key={p.key} className="flex items-center gap-2 text-xs text-[#94A3B8] cursor-pointer px-2 py-1.5 rounded hover:bg-[#263248]">
      ...
      {list.length > 0 ? (
        <div className="space-y-3">
          {list.map((role: any) => {
            ...
            return (
              <div key={role.id} className="bg-[#1E293B] border border-[#334155] rounded-xl p-4">
                <div className="flex items-start justify-between">
                  <div>
                    <h3 className="font-semibold text-[#F1F5F9]">{role.name}</h3>
                    {role.description && <p className="text-xs text-[#64748B] mt-0.5">{role.description}</p>}
                    <p className="text-xs text-[#475569] mt-1">{enabledCount} permisos activos</p>
                  </div>
                ...
                <div className="flex flex-wrap gap-1.5 mt-3">
                  {ALL_PERMISSIONS.filter(p => perms[p.key]).map(p => (
                    <span key={p.key} className="px-2 py-0.5 rounded-full text-[10px] bg-[#3B82F6]/20 text-[#3B82F6]">
                      {p.label}
                    </span>
                  ))}
                </div>
        ...
        <div className="bg-[#1E293B] border border-[#334155] rounded-xl p-12 text-center">
          ...
          <p className="text-[#64748B] text-sm">Sin roles personalizados. Los roles base (admin/agente/cliente) siguen activos.</p>
        </div>
      )}
    </div>
  )
