    ...
    <>
      ...
      {open && (
        <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4">
          <div className="bg-[#1E293B] border border-[#334155] rounded-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between px-5 py-3 border-b border-[#334155]">
              <h3 className="text-sm font-semibold text-[#F1F5F9]">Nueva macro</h3>
              <button onClick={() => setOpen(false)} className="text-[#64748B] hover:text-[#F1F5F9]"><X size={16} /></button>
            </div>
            <form action={handleSubmit} className="p-5 space-y-4">
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <label className="text-xs text-[#94A3B8]">Acciones</label>
                ...
                {actions.length === 0 && (
                  <p className="text-xs text-[#475569] text-center py-4 border border-dashed border-[#334155] rounded-lg">
                ...
                <div className="space-y-2">
                  {actions.map((a, i) => (
                    <div key={i} className="flex items-start gap-2 p-3 bg-[#0F172A] border border-[#334155] rounded-lg">
                      <div className="flex-1 grid grid-cols-2 gap-2">
                        <select value={a.type} onChange={e => updateAction(i, 'type', e.target.value)}
                          className="px-2 py-1.5 bg-[#1E293B] border border-[#334155] rounded text-xs text-[#94A3B8] focus:outline-none">
                        ...
                        {(a.type === 'add_tag' || a.type === 'add_comment') && (
                          <input value={a.value} onChange={e => updateAction(i, 'value', e.target.value)}
                            ...
                            className="px-2 py-1.5 bg-[#1E293B] border border-[#334155] rounded text-xs text-[#F1F5F9] placeholder-[#475569] focus:outline-none" />
                        )}
                      </div>
                      <button type="button" onClick={() => removeAction(i)}
                        className="text-[#475569] hover:text-[#EF4444] mt-1">
                        <Trash2 size={13} />
                      </button>
                    </div>
                  ))}
              ...
              <div className="flex justify-end gap-2 pt-2">
                <button type="button" onClick={() => setOpen(false)}
                  className="px-4 py-2 rounded-lg text-sm text-[#94A3B8] border border-[#334155] hover:border-[#475569] transition-colors">
                  Cancelar
                </button>
